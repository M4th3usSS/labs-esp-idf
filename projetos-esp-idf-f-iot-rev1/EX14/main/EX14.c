...
#define SERVER_IP 	"www.cientistadofuturo.com.br"
...
void Task_httpPOST ( void *pvParameter )
{
    char response[HTTP_JSON_RETURN_SIZE] = {0}; 
	int error;
	char * req = NULL;
	uint32_t counter;
	...
	{	
		...
		const char * mesg_POST = \
		"POST /site/query_insert_db.php HTTP/1.1\r\n"
		"HOST: www.cientistadofuturo.com.br\r\n"
		"content-type: application/x-www-form-urlencoded\r\n"
		"content-length: ";
		
		char databody[50];
		sprintf(databody, "nome=Contador&valor=%d&timestamp=0", counter);
		
		req = (char*)pvPortMalloc(512);
        ...
        sprintf(req, "%s%d\r\n\r\n%s\r\n\r\n", mesg_POST, strlen(databody), databody); 			
...
void Task_httpGET ( void *pvParameter )
{
    char response[HTTP_JSON_RETURN_SIZE] = {0}; 
	int error;
	char * req = NULL;
	...
	{	
		...
		const char * mesg_GET = \
		"GET /site/query_select_by_page_db.php?page=1 HTTP/1.1\r\n"
		"HOST: www.cientistadofuturo.com.br\r\n"
		"content-type: application/x-www-form-urlencoded\r\n\r\n";
		
		req = (char*)pvPortMalloc(strlen(mesg_GET)+1);
...
{	
	...
	*/
	...
	//xTaskCreate( Task_httpPOST, "TaskHttpGet", 15000, NULL, 5, NULL );
	xTaskCreate( Task_httpGET, "TaskHttpGet", 15000, NULL, 5, NULL );
}
