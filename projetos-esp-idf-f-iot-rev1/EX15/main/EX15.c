...
#define SERVER_IP 	"18.208.42.219"
...
void Task_Socket ( void * pvParameter ) 
{
    ...
    for(;;) {
		...
		if(rc < 0) {
			ESP_LOGI(TAG, "accept: %d %s", sock, strerror(errno));
		} else {
			xSocket.sock = sock; 
...
void Task_SendReceive ( void * pvParameter ) 
{
	...
	Socket * xSocket = (Socket*) pvParameter;
	...
	char databody[50];
	sprintf(databody, "nome=Contador&valor=%d&timestamp=0", xSocket->counter);
	...
	int rc = send(xSocket->sock, req, strlen(req), 0);
	ESP_LOGI(TAG, "send: rc: %d", rc);
	
	for(;;)
	{
	  
		ssize_t sizeRead = recv(xSocket->sock, data+rec_offset, total-rec_offset, 0);
		if (sizeRead < 0) {
		...
		if(sizeRead > 0 ) {	 
		   ESP_LOGI(TAG, "Socket: %d - Data read (size: %d) was: %.*s", xSocket->sock, sizeRead, sizeRead, data);
		   rec_offset += sizeRead; 
		 }
...
{	
	...
	*/
	...
	xTaskCreate( Task_Socket, "TaskSocket", 10000, NULL, 5, NULL );
}
