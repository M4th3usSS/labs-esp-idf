 ...
 * Variáveis
 */
QueueHandle_t Queue_http_post;
QueueHandle_t Queue_http_get;

static EventGroupHandle_t wifi_event_group;
const int WIFI_CONNECTED_BIT = BIT0;
static const char *TAG = "main: ";

SemaphoreHandle_t semaforo;//Objeto do semaforo
/**
 * Protótipos
 */
static esp_err_t event_handler(void *ctx, system_event_t *event);
void wifi_init_sta( void );
void task_http_post ( void *pvParameter );
void task_http_get ( void *pvParameter );
...
void task_http_post ( void *pvParameter )
{
	...
	{	
		 ...
		 */
	    xQueueReceive( Queue_http_post, &count, portMAX_DELAY ); 
		...
		char databody[50];
		sprintf( databody, "{\"value\":%d}", count );
...
void task_http_get ( void *pvParameter )
{
	...
	{	
		 ...
		 * Aguarda o botão ser pressionado;
		 */
	    xQueueReceive( Queue_http_get, &count, portMAX_DELAY ); 
    ...
    for(;;) 
	{
		...
		{ 
			...
			*/
			...
			if( gpio_get_level( BUTTON ) == 0 && aux == 0 ) 
			{
				if ( xSemaphoreTake( semaforo, pdMS_TO_TICKS(200) ) == pdPASS )
 				{
					if( xQueueSend( Queue_http_post, (void *)&count, (TickType_t )0 ) )
					{
						count++;
					}	
...
/**
 * Task responsável pelo pulling via GET; 
 * A cada intervalo de 20seg é realizado uma requisição GET via HTTP;
 */
void task_get_pull( void *pvParameter )
{
	int count;

	if( DEBUG )
		ESP_LOGI( TAG, "task_get_pull run...\n" );

	for(;;)
	{
		if ( xSemaphoreTake( semaforo, pdMS_TO_TICKS(200) ) == pdPASS )
 		{
			xQueueSend( Queue_http_get, (void *)&count, portMAX_DELAY );
			count++; 
...
{	
	...
	*/
	 ...
	 */
	Queue_http_get = xQueueCreate( 5, sizeof(uint32_t) );
	Queue_http_post = xQueueCreate( 5, sizeof(uint32_t) );
	...
	*/
	 ...
	 * Task responsável pelas rotinas de tratamento de button;
	 */
	if( xTaskCreate( task_get_pull, "task_get_pull", 2048, NULL, 5, NULL ) != pdTRUE )
	{
		if( DEBUG )
			ESP_LOGI( TAG, "error - nao foi possivel alocar task_get_pull.\n" );	
		return;		
	}
	 ...
	 */
	if( xTaskCreate( task_http_get, "task_http_get", 10000, NULL, 5, NULL ) != pdTRUE )
	{
		if( DEBUG )
			ESP_LOGI( TAG, "error - nao foi possivel alocar task_http_get.\n" );	
		return;		
	}

	/**
	 * Task responsável pelas requisições (POST) http;
	 */
	if( xTaskCreate( task_http_post, "task_http_post", 10000, NULL, 5, NULL ) != pdTRUE )
	{
		if( DEBUG )
			ESP_LOGI( TAG, "error - nao foi possivel alocar task_http_post.\n" );	
		return;		
	}
}
